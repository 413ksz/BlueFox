        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {features.map((feature, index) => (
            <Motion.div
              ...
              className={
                "p-6 rounded-xl border border-gray-800 shadow-lg bg-gray-900/80 backdrop-blur-md hover:border-blue-500/30 hover:shadow-blue-500/20 transition-all duration-300 flex flex-col items-center text-center space-y-4 group"
              }
            >
              {mounted() && (
                <feature.icon
                  className="w-10 h-10 text-blue-400 transition-transform group-hover:scale-110"
                  aria-hidden="true"
                />
              )}
