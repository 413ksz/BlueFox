...
func UserLoginHandler(w http.ResponseWriter, r *http.Request) {
	...
	result := db.Where("email = ? AND password = ?", user.Email, user.Password).Preload("ProfilePictureAsset").First(&user).Select("id", "username", "profile_picture_asset_id")
	if result.Error != nil {
	...
	token, err := jwt_token.GenerateJWTToken(user.Username, user.ID.String(), user.ProfilePictureAssetID.String())
